...
import { formatCurrencyBRL, getFromStorage, setToStorage } from '../../../core/functionUtils.js';
...
export class CheckoutManager {
    ...
    _initializeElements() {
        ...
        this._populateInstallments();
    }

    /**
     * Preenche as opções de parcelas do cartão de crédito
     * @private
     */
    _populateInstallments() {
        const select = this.elements.creditCardForm.installments;
        if (!select) return;
        select.innerHTML = '';
        const total = this.orderSummary ? this.orderSummary.total : 0;
        for (let i = 1; i <= 12; i++) {
            const value = total / i;
            const option = document.createElement('option');
            option.value = i;
            option.textContent = window.i18n ? window.i18n.translate('installment_option', { n: i, value: formatCurrencyBRL(value) }) : `${i}x de ${formatCurrencyBRL(value)} sem juros`;
    ...
    _setupEventListeners() {
        ...
        if (this.elements.backToShopping) {
            this.elements.backToShopping.addEventListener('click', () => {
                ...
                const carts = getFromStorage('cartsByUser', {});
                if (carts[userKey]) {
                    carts[userKey] = [];
                    setToStorage('cartsByUser', carts);
                }
    ...
    _calculateOrderSummary() {
        ...
        setTimeout(() => this._populateInstallments(), 0);
        return {
            subtotal,
    ...
    _goToReviewStep() {
        ...
        this._populateInstallments(); // Atualiza parcelas ao revisar
    }
        ...
        this.cartItems.forEach(item => {
            ...
            orderItemElement.innerHTML = `
                ...
                <div class="order-item-info">
                    ...
                    <div>Qtd: ${item.quantity} x <span class="price">${formatCurrencyBRL(discountedPrice)}</span></div>
                </div>
                <div class="order-item-total">${formatCurrencyBRL(discountedPrice * item.quantity)}</div>
            `;
            
            this.elements.orderItemsList.appendChild(orderItemElement);
        });
        
        this.elements.orderSubtotal.textContent = formatCurrencyBRL(this.orderSummary.subtotal);
        this.elements.orderShipping.textContent = formatCurrencyBRL(this.orderSummary.shipping);
        this.elements.orderDiscount.textContent = formatCurrencyBRL(this.orderSummary.discount);
        this.elements.orderTotal.textContent = formatCurrencyBRL(this.orderSummary.total);
    ...
    _showConfirmation(paymentResult) {
        ...
        this.elements.confirmationTotal.textContent = formatCurrencyBRL(this.orderSummary.total);
    }
